...
class CategoryPointCache:
    ...
    async def get_codes(self) -> tuple[CategoryCode, ...]:
        if (cached := await CategoryPointRedisRepository.get(self.cache_key)) is not None:
            return cached
    ...
    async def _set_cache(self, codes: tuple[CategoryCode, ...]) -> None:
        try:
            ...
            logger.error(f"[CACHE] Failed to insert category point cache to mongodb: {self.cache_key}", exc_info=True)
        except RedisError:
            logger.error(f"[CACHE] Failed to insert category point cache to redis: {self.cache_key}", exc_info=True)
